...
extern	int		SetEmuration320x240( int Flag ) ;							// ６４０ｘ４８０の画面で３２０ｘ２４０の画面解像度にするかどうかのフラグをセットする、６４０ｘ４８０以外の解像度では無効( TRUE:有効  FALSE:無効 )
...
extern	int		SaveDrawScreenToJPEG( int x1, int y1, int x2, int y2, const char *FileName, int Quality = 80 , int Sample2x1 = TRUE ) ;				// 現在描画対象になっている画面をＪＰＥＧ形式で保存する
extern	int		SaveDrawScreenToPNG( int x1, int y1, int x2, int y2, const char *FileName, int CompressionLevel = -1 ) ;							// 現在描画対象になっている画面をＰＮＧ形式で保存する
...
extern	int			SetTransformToWorld( MATRIX *Matrix, int SetIndex = 0 ) ;			// ワールド変換用行列をセットする
...
extern	int			SetTextureAddressMode( int Mode /* DX_TEXADDRESS_WRAP 等 */ ) ;		// テクスチャアドレスモードを設定する
...
extern	int			CreatePerspectiveFovMatrixRH( MATRIX *Out, float fov, float zn, float zf ) ;				// 射影行列を作成する(右手座標系用)
...
extern 	int		PlayMovieToGraph( int GraphHandle, int PlayType = DX_PLAYTYPE_BACK ) ;							// ムービーグラフィックに含まれるムービーの再生を開始する
...
extern	int			MV1AttachMotion( int MHandle, int MV1ModelHandle, const char *MotionName, int MotionIndex = -1, int AttachIndex = 0 ) ;	// モーションをアタッチする
extern	int			MV1DetachMotion( int MHandle, int AttachIndex = 0 ) ;									// モーションをデタッチする

extern	int			MV1PlayMotion( int MHandle, int Loop, int AttachIndex = 0 ) ;							// モーションを再生する
extern	int			MV1MotionAddTime( int MHandle, float AddTime, int AttachIndex = 0 ) ;					// モーションを進める
extern	int			MV1StopMotion( int MHandle, int AttachIndex = 0 ) ;										// モーションを止める
extern	int			MV1GetMotionState( int MHandle, int AttachIndex = 0 ) ;									// モーションが再生中かどうかを取得する( TRUE:再生中  FALSE:停止中 )

extern	int			MV1SetPosition( int MHandle, float x, float y, float z ) ;								// モデルの位置をセット
extern	int			MV1SetScale( int MHandle, float x, float y, float z ) ;									// モデルのスケールをセット
extern	int			MV1SetRotation( int MHandle, float x, float y, float z ) ;								// モデルのローテーションをセット
extern	int			MV1DrawModel( int MHandle ) ;															// モデルの描画
